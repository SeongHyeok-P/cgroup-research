...
static const char *condition_name(enum condition condition)
{
    ...
    case CONDITION_UNCONTROLLED:
        return "uncontrolled";
    case CONDITION_CPU75:
        return "cpu75";
    case CONDITION_CPU50:
        return "cpu50";
    case CONDITION_CPU25:
        return "cpu25";
    case CONDITION_CPU10:
        return "cpu10";
    default:
        return "unknown";
    }
...
static unsigned int condition_cpu_allow_pct(enum condition condition)
{
    switch (condition) {
    case CONDITION_UNCONTROLLED:
        return 100U;
    ...
    enum condition *out)
{
    ...
    } else if (strcmp(text, "cpu75") == 0) {
        *out = CONDITION_CPU75;
    } else if (strcmp(text, "cpu50") == 0) {
        *out = CONDITION_CPU50;
    } else if (strcmp(text, "cpu25") == 0) {
        *out = CONDITION_CPU25;
    } else if (strcmp(text, "cpu10") == 0) {
        *out = CONDITION_CPU10;
    } else {
        return -1;
    }
...
static void config_default(struct config *cfg)
{
    ...
    cfg->order[2] = CONDITION_CPU75;
    cfg->order[3] = CONDITION_CPU50;
    cfg->order[4] = CONDITION_CPU25;
    cfg->order[5] = CONDITION_CPU10;
}
...
static void usage(const char *program)
{
    fprintf(stderr,
            ...
            "\n"
            "Experiment F: all five background candidates run together.\n"
            "Only the highest-overlap (o100) candidate is throttled.\n"
            "cpu75/cpu50/cpu25/cpu10 mean 75/50/25/10 percent CPU allowance\n"
            "for o100 via cpu.max; the other four candidates remain unrestricted.\n",
            program,
            program);
}
    ...
    struct condition_result *out)
{
    ...
    size_t i;
    unsigned int allow_pct;
    unsigned int quota_us = 0U;
    bool throttle_o100 = false;
    ...
    allow_pct = condition_cpu_allow_pct(condition);

    if (condition >= CONDITION_CPU75 &&
        condition <= CONDITION_CPU10) {
        uint64_t quota64 =
            (uint64_t)cfg->throttle_period_us * (uint64_t)allow_pct / 100ULL;

        if (quota64 == 0ULL || quota64 > UINT_MAX) {
            return -1;
        }

        quota_us = (unsigned int)quota64;
        throttle_o100 = true;
    }

    if (out->has_background) {
        for (i = 0U; i < CANDIDATE_COUNT; ++i) {
            bool throttled = (i == CANDIDATE_COUNT - 1U) && throttle_o100;
    ...
    if (out->has_background) {
        for (i = 0U; i < CANDIDATE_COUNT; ++i) {
            ...
            if (throttled) {
                printf("[CONTROL] %s_cpu_max=%u %u allow_pct=%u\n",
                       candidate_name(i),
                       quota_us,
                       cfg->throttle_period_us,
                       allow_pct);
            } else {
                printf("[CONTROL] %s_cpu_max=unrestricted\n",
                       candidate_name(i));
            }
        }
...
int main(int argc, char **argv)
{
    ...
    printf("============================================================\n");
    printf("Experiment F: cpu.max strength sweep v1\n");
    printf("============================================================\n");
    ...
    printf("[CONFIG] throttle_period_us=%u\n",
           cfg.throttle_period_us);
    printf("[CONFIG] o100_cpu_allow_levels=100,75,50,25,10 percent\n");
    ...
    printf("\nRESULT_CSV_HEADER,condition,o100_cpu_allow_pct,"
           "victim_elapsed_sec,victim_reads,victim_ns_per_read,"
           "o0_Mload_per_sec,o25_Mload_per_sec,o50_Mload_per_sec,"
           "o75_Mload_per_sec,o100_Mload_per_sec,total_bg_Mload_per_sec\n");
    ...
    for (i = 0U; i < CONDITION_COUNT; ++i) {
        ...
        printf("[RESULT] o100_cpu_allow_pct=%u\n",
               condition_cpu_allow_pct(condition));
        ...
        printf("RESULT_CSV,%s,%u,%.6f,%" PRIu64 ",%.3f,"
               "%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n",
               condition_name(condition),
               condition_cpu_allow_pct(condition),
               result.victim.elapsed_sec,
...

    {
        ...
        double o100_bg[CONDITION_COUNT] = {0.0};
        double none_ns;
        ...
        for (c = CONDITION_CPU75; c <= CONDITION_CPU10; ++c) {
            double recovery = 0.0;
            double total_retention = 0.0;
            double o100_retention = 0.0;
            double bg_loss = 0.0;
            double recovery_per_bg_loss = 0.0;

            if (span > 0.0) {
                recovery = (uncontrolled_ns - latency[c]) / span * 100.0;
            }
            if (total_bg[CONDITION_UNCONTROLLED] > 0.0) {
                total_retention = total_bg[c] /
                    total_bg[CONDITION_UNCONTROLLED] * 100.0;
            }
            if (o100_bg[CONDITION_UNCONTROLLED] > 0.0) {
                o100_retention = o100_bg[c] /
                    o100_bg[CONDITION_UNCONTROLLED] * 100.0;
            }

            bg_loss = 100.0 - total_retention;
            if (bg_loss > 0.0) {
                recovery_per_bg_loss = recovery / bg_loss;
            }

            printf("[COMPARE] %s_o100_cpu_allow_pct=%u\n",
                   condition_name((enum condition)c),
                   condition_cpu_allow_pct((enum condition)c));
            ...
            printf("[COMPARE] %s_total_bg_retention_pct=%.3f\n",
                   condition_name((enum condition)c), total_retention);
            printf("[COMPARE] %s_o100_retention_pct=%.3f\n",
                   condition_name((enum condition)c), o100_retention);
            printf("[COMPARE] %s_recovery_per_total_bg_loss=%.3f\n",
                   condition_name((enum condition)c), recovery_per_bg_loss);
        }
    }
    ...
    if (cfg.self_test) {
        ...
        printf("  - none/uncontrolled/cpu75/cpu50/cpu25/cpu10 completed\n");
        printf("  - only the o100 candidate was throttled in sweep conditions\n");
